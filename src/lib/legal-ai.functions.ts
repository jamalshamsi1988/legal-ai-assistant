...
دستورالعمل ویژه:
- بخش draft باید یک لایحه/شکایت‌نامه کامل و رسمی با ۳۰۰۰ تا ۵۰۰۰ کلمه باشد. کوتاه‌تر از ۳۰۰۰ کلمه قابل قبول نیست.
- شامل: مقدمه، شرح کامل وقایع، استدلالات مفصل با استناد دقیق به مواد قانونی، پاسخ به دفاعیات احتمالی طرف مقابل، خواسته‌ها، نتیجه‌گیری.
- از قالب رسمی لوایح دادگاه‌های ایران («ریاست محترم دادگاه...، احتراماً به استحضار می‌رساند...») استفاده کن.
- بخش analysis نیز حداقل ۸۰۰ کلمه و کامل باشد.`;
  ...
  .handler(async ({ data }): Promise<LegalAnalysis> => {
    ...
    try {
      const res = await generateText({
        ...
        maxOutputTokens: data.detailed ? 16000 : 4000,
      });
      text = res.text ?? "";
    } catch (e: unknown) {
